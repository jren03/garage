...
import garage.mbrl.env
import garage.mbrl.planning
import garage.mbrl.types
import garage.mbrl.env.mujoco_envs


def _get_term_and_reward_fn(
    cfg: Union[omegaconf.ListConfig, omegaconf.DictConfig],
) -> Tuple[garage.mbrl.types.TermFnType, Optional[garage.mbrl.types.RewardFnType]]:
    import mbrl.env
...
def _handle_learned_rewards_and_seed(
    cfg: Union[omegaconf.ListConfig, omegaconf.DictConfig],
    env: gym.Env,
    reward_fn: garage.mbrl.types.RewardFnType,
) -> Tuple[gym.Env, garage.mbrl.types.RewardFnType]:
...
def _legacy_make_env(
    ...
    gym.Env, garage.mbrl.types.TermFnType, Optional[garage.mbrl.types.RewardFnType]
]:
    if cfg.overrides.env == "ant_truncated_obs":
        env = garage.mbrl.env.mujoco_envs.AntTruncatedObsEnv()
        term_fn = garage.mbrl.env.termination_fns.ant
        reward_fn = None
        env = gym.wrappers.TimeLimit(
            env, max_episode_steps=cfg.overrides.get("trial_length", 1000)
        )
    elif cfg.overrides.env == "humanoid_truncated_obs":
        env = garage.mbrl.env.mujoco_envs.HumanoidTruncatedObsEnv()
        term_fn = garage.mbrl.env.termination_fns.humanoid
        reward_fn = None
        env = gym.wrappers.TimeLimit(
            env, max_episode_steps=cfg.overrides.get("trial_length", 1000)
        )
        ...
        if hasattr(cfg.overrides, "term_fn"):
...
class EnvHandler(ABC):
    ...
    def rollout_env(
        ...
        Args:
            ...
            agent (:class:`garage.mbrl.planning.Agent`, optional): if given, an agent to obtain actions.
