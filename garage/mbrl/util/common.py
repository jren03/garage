...
import garage.mbrl.models
import garage.mbrl.planning
import garage.mbrl.types
...
def create_one_dim_tr_model(
    ...
    :class:`garage.mbrl.models.OneDTransitionRewardModel` (see its documentation for explanation of some
    ...
    Returns:
        (:class:`garage.mbrl.models.OneDTransitionRewardModel`): the model created.

    """
    # This first part takes care of the case where model is BasicEnsemble and in/out sizes
    # are handled by member_cfg
    model_cfg = cfg.algorithm.dynamics_model
    if model_cfg._target_ == "garage.mbrl.models.BasicEnsemble":
    ...
    model = hydra.utils.instantiate(cfg.algorithm.dynamics_model)
    ...
    if name_obs_process_fn:
        ...
        obs_process_fn = None
    dynamics_model = garage.mbrl.models.OneDTransitionRewardModel(
        model,
...
def create_one_dim_tr_model_value(
    ...
    if name_obs_process_fn:
        ...
        obs_process_fn = None
    dynamics_model = garage.mbrl.models.ValueOneDTransitionRewardModel(
        model,
        target_is_delta=cfg.algorithm.target_is_delta,
        normalize=cfg.algorithm.normalize,
        normalize_double_precision=cfg.algorithm.get(
...
def create_replay_buffer(
    ...
    if not dataset_size:
        dataset_size = cfg.overrides.total_env_steps
...
def train_model_and_save_model_and_data(
    model: garage.mbrl.models.Model,
    model_trainer: garage.mbrl.models.ModelTrainer,
    cfg: omegaconf.DictConfig,
    replay_buffer: ReplayBuffer,
    ...
    Args:
        model (:class:`garage.mbrl.models.Model`): the model to train.
        model_trainer (:class:`garage.mbrl.models.ModelTrainer`): the model trainer.
        ...
        callback (callable, optional): if provided, this function will be called after
            every training epoch. See :class:`garage.mbrl.models.ModelTrainer` for signature.
    """
    dataset_train, dataset_val = garage.mbrl.util.common.get_basic_buffer_iterators(
        replay_buffer,
...
def rollout_model_env(
    model_env: garage.mbrl.models.ModelEnv,
    initial_obs: np.ndarray,
    plan: Optional[np.ndarray] = None,
    agent: Optional[garage.mbrl.planning.Agent] = None,
    num_samples: int = 1,
    ...
    Args:
         model_env (:class:`garage.mbrl.models.ModelEnv`): the dynamics model environment to simulate.
         initial_obs (np.ndarray): initial observation to start the episodes.
         plan (np.ndarray, optional): sequence of actions to execute.
         agent (:class:`garage.mbrl.planning.Agent`): an agent to generate a plan before
...
def rollout_agent_trajectories(
    ...
    agent: garage.mbrl.planning.Agent,
    agent_kwargs: Dict,
    ...
    Args:
        ...
        agent (:class:`garage.mbrl.planning.Agent`): the agent used to generate an action.
