...
class ValueOneDTransitionRewardModel(Model):
    ...
    def loss(
        ...
        return self.model.loss(model_in, target=target, value=True, agent=agent)
    ...
    def update(
        ...
        model_in, target = self._process_batch(batch)
        #print(self.model)
        return self.model.update_value(agent, model_in, optimizer, target=target)
    ...
    def eval_score(
        ...
        with torch.no_grad():
            model_in, target = self._process_batch(batch)
            return self.model.eval_score(model_in, target=target, value=True, agent=agent)
