...
class ModelTrainerValue:
    ...
    def train(
        ...
        best_val_score = self.evaluate(agent,eval_dataset) if evaluate else None
        ...
        for epoch in epoch_iter:
            ...
            for batch in tqdm.tqdm(dataset_train, disable=disable_tqdm):
                loss, meta = self.model.update(agent,batch, self.optimizer)
            ...
            if evaluate:
                eval_score = self.evaluate(
                    agent, eval_dataset, batch_callback=batch_callback_epoch
                )
    ...
    def evaluate(
        self, agent, dataset: TransitionIterator, batch_callback: Optional[Callable] = None
    ) -> torch.Tensor:
        ...
        for batch in dataset:
            batch_score, meta = self.model.eval_score(agent, batch)
