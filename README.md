...
```
python experts/download_datasets.py --env <env_name>
```
for a specific environment, or 
```
python experts/download_datasets.py --all
```
for all environments (`Ant-v3`, `Hopper-v3`, `Humanoid-v3`, `Walker-v3`). To acquire the datasets used in `antmaze` experiments, please reference the following section.  
...
```
Demonstrations will be saved as an `.npz` file containing the following entries: `observations`, `actions`, `next_observations`, `rewards`, `terminals`, `timeouts`, `qpos`, and `qvel`. 
